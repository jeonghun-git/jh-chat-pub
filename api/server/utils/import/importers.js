...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing jh-chat conversation');
...
/**
 * Imports a jh-chat conversation from JSON.
 *
...
async function importLibreChatConvo(
  ...
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid jh-chat file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from jh-chat file`, error);
  }
}
